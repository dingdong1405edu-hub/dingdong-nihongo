...
export async function GET() {
  ...
  try {
    ...
    const msg = e instanceof Error ? e.message : 'Failed to fetch vocab units'
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const unit = await db.vocabUnit.create({ data })
    return NextResponse.json({ ok: true, data: unit }, { status: 201 })
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Failed to create vocab unit'
