...
const registerSchema = z
  .object({
    name: z.string().min(2, 'Tên tối thiểu 2 ký tự').max(60, 'Tên tối đa 60 ký tự'),
...
export default function RegisterPage() {
  ...
  async function onSubmit(values: RegisterValues) {
    ...
    // Build FormData to match existing server action signature
    ...
    try {
      const mod = await import('@/server/actions/auth')
      const result = await mod.register(formData)
    ...
    <div className="w-full max-w-md">
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-zinc-100 p-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-zinc-500">Tạo tài khoản mới và bắt đầu học tiếng Nhật</p>
        </div>
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 border border-zinc-200 bg-white hover:bg-zinc-50 text-zinc-700 font-medium py-2.5 px-4 rounded-xl transition-colors"
        >
