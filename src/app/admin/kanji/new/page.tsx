...
export default function NewKanjiPage() {
  ...
  const [radicals, setRadicals] = useState<Radical[]>([{ radical: '', meaning: '' }])
  const [examples, setExamples] = useState<Example[]>([{ word: '', reading: '', meaning: '' }])
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to create kanji')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="max-w-2xl space-y-5">
      <div>
        <h2 className="text-xl font-semibold text-zinc-900">Thêm Kanji mới</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="rounded-xl border border-zinc-200 bg-white p-5 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input
                ...
                maxLength={1}
                placeholder="食"
                className="w-full rounded-md border border-zinc-300 px-3 py-2 text-center text-xl outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                style={{ fontFamily: "'Noto Sans JP', sans-serif" }}
              />
            </div>
            <div>
              <label className="mb-1 block text-sm font-medium text-zinc-700">
                JLPT <span className="text-red-500">*</span>
              </label>
              <select
                name="jlptLevel"
                required
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-zinc-700">Grade (lớp)</label>
              <input
                name="grade"
                type="number"
                min="1"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="mb-1 block text-sm font-medium text-zinc-700">
                Meaning (VN) <span className="text-red-500">*</span>
              </label>
              <input
                name="meaning"
                required
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-zinc-700">
                Stroke Count <span className="text-red-500">*</span>
              </label>
              <input
                name="strokeCount"
                type="number"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="mb-1 block text-sm font-medium text-zinc-700">
                Onyomi (phân tách bằng dấu phẩy)
              </label>
              <input
                name="onyomi"
                placeholder="ショク、ジキ"
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-zinc-700">
                Kunyomi (phân tách bằng dấu phẩy)
              </label>
              <input
                name="kunyomi"
          ...
          <div>
            ...
            <textarea
              name="mnemonic"
              rows={2}
              placeholder="Kanji 食 trông như một người đang ngồi ăn cơm..."
              className="w-full rounded-md border border-zinc-300 px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
            />
          </div>
        </div>
        ...
        <div className="rounded-xl border border-zinc-200 bg-white p-5 space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-zinc-700">Radicals (Bộ thủ)</h3>
            <button
              type="button"
              onClick={addRadical}
        ...
        <div className="rounded-xl border border-zinc-200 bg-white p-5 space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-zinc-700">Examples (Ví dụ từ)</h3>
            <button
              type="button"
              onClick={addExample}
              className="text-xs text-indigo-600 hover:underline"
